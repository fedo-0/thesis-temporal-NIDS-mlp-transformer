...
def clean_and_split_dataset(dataset_path, config_path, output_dir,
    ...
    if removed_classes:
        print(f"\nDataset filtrato: rimosse {len(removed_classes)} classi rare")
    ...
    if dataset_path != "resources/datasets/NF-UNSW-NB15-v3.csv":
        ...
        if total_removed_samples > 0:
            print(f"\nApplicato undersampling uniforme: rimosse {total_removed_samples:,} campioni")
    ...
    if total_after_split != len(df_clean):
        print(f"!!!!! Differenza: {len(df_clean) - total_after_split:,} campioni !!!!!!")
