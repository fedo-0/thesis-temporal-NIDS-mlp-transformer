...
def create_sliding_windows_indices(df, window_size=10):
    ...
    progress_interval = max(1, (total_len // window_size) // 100)
...
def round_robin_window_assignment(sliding_windows, train_ratio=0.70, val_ratio=0.15, test_ratio=0.15):
    ...
    cycle_length = 20
    train_per_cycle = int(cycle_length * train_ratio)
    val_per_cycle = int(cycle_length * val_ratio)
    test_per_cycle = cycle_length - train_per_cycle - val_per_cycle
    ...
    for i, window in enumerate(sliding_windows):
        ...
        if cycle_pos < train_per_cycle:
            train_windows.append(window)
        elif cycle_pos < train_per_cycle + val_per_cycle:
